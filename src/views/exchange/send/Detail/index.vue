<template>
  <page-container :showBack="true">
    ...
    <FullPage>
      <div class="detail">
        <div class="sider">
          <Left :productListData="productListData" @select-data="selectData" @searchValue="searchValue" />
        </div>
        <div class="right"><j-tabs v-model:activeKey="activeKey" class="tabs">
            <j-tab-pane class="tab_con" key="DataMap" tab="物模型映射">
              <DataMap :mapDataList="mapDataList" :dataMapOpt="dataMapOpt" :deviceDetailList="deviceDetailList"
                :dataDetailList="dataDetailList" :sendId="sendId" :selectProductId="selectProductId"
            ...
            <j-tab-pane class="tab_con" key="DeviceMap" tab="设备映射">
              <DeviceMap :deviceIdsMap="deviceIdsMap" :deviceIdsMapOpt="deviceIdsMapOpt"
                :deviceDetailList="deviceDetailList" :dataDetailList="dataDetailList" :allDataMapping="allDataMapping"
...
const searchValue = (newValue: any) => {
  ...
  queryDeviceProductList(query).then((res: any) => {
    if (res.result) {
      ...
      selectProductId.value = undefined
