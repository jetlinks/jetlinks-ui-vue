...
import BindParentDevice from '@/views/device/Instance/Detail/components/BindParentDevice/index.vue';
...
const Status = defineComponent({
    ...
    emits: ['percentChange', 'countChange', 'stateChange', 'showFix'],
    setup(props, { emit, expose }) {
        ...
        const diagnoseNetworkRes = ref(false);
        const diagnoseGatewayRes = ref(false);
        const diagnoseProductRes = ref(false);
        const diagnoseDeviceRes = ref(false);
        const diagnoseProductAuthConfigRes = ref(false);
        const diagnoseDeviceAuthConfigRes = ref(false);
        const diagnoseParentDeviceRes = ref(false);
        const diagnoseCTWingRes = ref(false);
        const diagnoseOnenetRes = ref(false);
        ...
        const networkDevice = [
            ...
            diagnoseProductRes,
            diagnoseDeviceRes,
            diagnoseProductAuthConfigRes,
            diagnoseDeviceAuthConfigRes,
        ];

        const childDevice = [
            diagnoseNetworkRes,
            diagnoseParentDeviceRes,
            diagnoseProductRes,
            diagnoseDeviceRes,
            diagnoseProductAuthConfigRes,
            diagnoseDeviceAuthConfigRes,
                ...
                if (unref(device).state?.value === 'online') {
                    ...
                    if (_device.accessId) {
                        const response: Record<string, any> =
                            await queryGatewayState(_device.accessId);
                        if (response.status === 200) {
                            gateway.value = response.request;
                            const address =
                                response.result?.channelInfo?.addresses || [];
                            const _label = address.some(
                                (i: any) => i.health === -1,
                            );
                            const __label = address.every(
                                (i: any) => i.health === 1,
                            );
        ...
        const diagnoseGateway = () =>
            new Promise(async (resolve) => {
                const desc =
                    props.providerType &&
                    ['child-device', 'cloud'].includes(props.providerType)
                ...
                if (unref(device).state.value === 'online') {
                    ...
                    if (!unref(gateway)?.id) {
                        ...
                        if (accessId) {
                            const response: Record<string, any> =
                                await queryGatewayState(accessId);
                            if (response.status === 200) {
                                gateway.value = response.result;
                                if (
                                    response.result?.state?.value === 'enabled'
                                ) {
                                    if (
                                        props.providerType === 'cloud' ||
                        ...
                        if (unref(gateway)?.state?.value === 'enabled') {
                            if (
                                props.providerType === 'cloud' ||
                                unref(device)?.accessProvider === 'gb28181-2016'
                            ) {
                                _item = {
                                    key: 'gateway',
                                    name: '设备接入网关',
            ...
            new Promise(async (resolve) => {
                if (unref(device).state.value === 'online') {
                    setTimeout(() => {
                        ...
                        diagnoseParentDeviceRes.value = true;
                        resolve({});
                    }, time);
                } else {
                    const _device = unref(device);
                    if (!_device?.parentId) {
                        ...
                        if (response.status === 200) {
                            if (
                                response?.result?.state?.value === 'notActive'
                            ) {
                                _item = {
                                    key: 'parent-device',
                                    name: '网关父设备',
                                ...
                                response?.result?.state?.value === 'online'
                            ) {
                                _item = {
                                    key: 'parent-device',
                                    name: '网关父设备',
            ...
            new Promise(async (resolve) => {
                if (unref(device).state?.value === 'online') {
                    ...
                    if (_device.productId) {
                        const response: Record<string, any> =
                            await queryProductState(_device.productId);
                        if (response.status === 200) {
        ...
        const diagnoseProductAuthConfig = () =>
            new Promise(async (resolve) => {
                ...
                if (_device.productId) {
                    const response: Record<string, any> =
                        await queryProductConfig(_device.productId);
                        ...
                        response.result.map((item: any, i: number) => {
                            ...
                            if (unref(device).state?.value === 'online') {
                                setTimeout(() => {
                                    ...
                                    diagnoseProductAuthConfigRes.value = true;
                                    resolve({});
                                }, time);
                            } else if (
                                !isExit(
                                    properties,
                                    Object.keys(_configuration).filter(
                                        (k: string) => !!_configuration[k],
                                    ),
                                )
                            ) {
                                setTimeout(() => {
        ...
        const diagnoseDeviceAuthConfig = () =>
            new Promise(async (resolve) => {
                ...
                if (_device.id) {
                    const response: Record<string, any> =
                        await queryDeviceConfig(_device.id);
                    if (response.status === 200 && response.result.length > 0) {
                        ...
                        response.result.map((item: any, i: number) => {
                            const _list = [...list.value];
                            if (
                                !_.map(_list, 'key').includes(`device-auth${i}`)
                            ) {
                                list.value = modifyArrayList(
                                    list.value,
                                    {
                            ...
                            if (_device.state?.value === 'online') {
                                setTimeout(() => {
                                    ...
                                    diagnoseDeviceAuthConfigRes.value = true;
                                    resolve({});
                                }, time);
                            } else if (
                                !isExit(
        ...
        const diagnoseOnenet = () =>
            new Promise(async (resolve) => {
                ...
                if (_device?.accessProvider === 'OneNet') {
                    const response: any = await queryDeviceConfig(
                        _device?.id || '',
                    );
                    configuration.device = response.result;
                    const _configuration = _device?.configuration || {};
            ...
            new Promise(async (resolve) => {
                ...
                if (_device?.accessProvider === 'Ctwing') {
                    ...
                    if (
                        ...
                        (config &&
                            (config.ctwing_product_id || config.master_key))
                    ) {
                        item = {
                            key: `ctwing`,
                            name: `设备-CTWing配置`,
                ...
                if (props.providerType === 'network') {
                    item.push(
                        <Badge
                            status="default"
                            text={
                                (unref(gateway)?.channelInfo?.addresses || [])
                                    .length > 1 ? (
                                    <>
                                        请检查设备网络是否畅通，并确保设备已连接到以下地址之一:
                                        <div class="serverItem">
                                            {(
                                                unref(gateway)?.channelInfo
                                                    ?.addresses || []
                                            ).map((i: any) => (
                                                <span
                                                    style={{ marginLeft: 15 }}
                    ...
                    if (
                        ...
                        unref(device)?.accessProvider &&
                        gatewayList.includes(
                            unref(device).accessProvider as string,
                        )
                    ) {
                        const response: any = await queryProtocolDetail(
                            unref(device).protocol,
                            'MQTT',
                        );
                        if (response.status === 200) {
                    ...
                    if (unref(device)?.accessProvider === 'gb28181-2016') {
                        const address =
                            unref(gateway)?.channelInfo?.addresses[0];
                        if (address) {
                            item.push(
                                <Badge
                                    status="default"
        ...
        const handleFix = async () => {
            ...
            if (unref(product)?.state !== 1) {
                const resp = await _deployProduct(
                    unref(device).productId || '',
                );
                if (resp.status === 200) {
                    list.value = modifyArrayList(list.value, {
                        key: 'product',
            ...
            if (
                props.providerType === 'network' ||
                props.providerType === 'child-device'
            ) {
            ...
            if (
                props.providerType === 'child-device' &&
                unref(device)?.parentId
            ) {
                if (unref(parent)?.state?.value === 'notActive') {
                    const resp = await deployDevice(
                        unref(device)?.parentId || '',
                    );
                    if (resp.status === 200) {
                        list.value = modifyArrayList(list.value, {
        ...
        watch(
            ...
            () => {
                if (status.value === 'finish') {
                    ...
                    const arr = _.map(unref(list), 'status').filter(
                        (i) => i !== 'loading',
                    );
        ...
        return () => (
            <div class={styles['statusBox']}>
                <Space>
                    {status.value === 'finish' &&
                        unref(device).state?.value !== 'online' &&
                        emit('showFix')}
                </Space>
                <div class={styles['statusContent']}>
                    {list.value.map((item) => (
                        <div key={item.key} class={styles['statusItem']}>
                            ...
                            <div
                                class={styles['statusRight']}
                                style={{ color: TextColorMap.get(item.status) }}
                            >
                                {item?.text}
                            </div>
                        </div>
