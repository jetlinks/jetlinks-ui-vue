        ...
        <j-form ref="formRef" :model="formData">
          ...
          <div v-else class="data-table-boolean-item">
            <div class="data-table-boolean-item--value">
              <j-form-item :name="['rangeValue', 0]" :rules="[{ required: true, message: $t('Metrics.ValueItem.6916334-0')}]">
                <Item v-model:value="formData.rangeValue[0]" />
              </j-form-item>
            </div>
            <div style="line-height: 32px">-</div>
            <div class="data-table-boolean-item--value">
              <j-form-item :name="['rangeValue', 1]" :rules="[{ required: true, message: $t('Metrics.ValueItem.6916334-0')}, { validator: validator}]">
    ...
    switch (type) {
      ...
      case 'boolean':
        ...
        if (item) {
          ...
          return _value === 'true' ? $t('Metrics.ValueItem.6916334-2') : $t('Metrics.ValueItem.6916334-3')
        } else {
          return ''
        }
      default:
...
const validatorTip = () =>{
  let tip = $t('Metrics.ValueItem.6916334-4')
  if (['boolean', 'date'].includes(type!)) {
    tip = $t('Metrics.ValueItem.6916334-5')
  }
  return $t('Metrics.ValueItem.6916334-6', [tip])
}

const validator = (_: any, value: any) => {

  if (props.range && formData.rangeValue![0] >= formData.rangeValue![1]) {
    return Promise.reject($t('Metrics.ValueItem.6916334-7'))
  }
...
const typeValidator = (_: any, value: any) => {
  ...
  if (type === 'string' && value?.length > 64) {
    return Promise.reject($t('Metrics.ValueItem.6916334-8'))
  }
  return Promise.resolve()
}
