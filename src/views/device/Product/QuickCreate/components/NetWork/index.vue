<template>
    <a-modal title="网络组件" visible :width="1000" :maskClosable="false" @ok="emits('selectedNetWork', networkCurrent)" @cancel="emits('close')">
        <div class="header">
            <j-input-search allowClear style="margin-right: 8px;" placeholder="请输入" @search="networkSearch"></j-input-search>
        ...
        <div class="content">
            <a-row :gutter="[12, 12]" v-if="networkList.length > 0">
                <a-col v-for="item in networkList" :key="item.id" :span="8">
                    <AccessCard @checkedChange="checkedChange" :checked="networkCurrent?.id" :data="{
                        ...item,
        ...
        <AddNetWork v-if="addVisible" :type="NetworkTypeMapping.get(type)" @close="addVisible = false"  @refresh="queryNetworkList"/>
    </a-modal>
</template>

<script setup>
import { getNetworkList } from '@/api/link/accessConfig';
