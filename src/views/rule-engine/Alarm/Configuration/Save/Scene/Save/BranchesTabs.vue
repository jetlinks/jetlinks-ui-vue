      ...
      <a-tabs v-if="branchesGroup.length" v-model:activeKey="activeKey">
        <template v-if="showDetailBtn" #rightExtra>
          <a-button @click.stop="show = !show">
            {{ show ? $t('Save.BranchesTabs.4265311-0') : $t('Save.BranchesTabs.4265311-1')}}
          </a-button>
        </template>
        <a-tab-pane
        ...
        >
          <div v-for="(branch, index) in group.children">
            <div style="display: flex;align-items: center" v-if="show">
              <span v-if="branch.when" style="padding-right: 12px;font-weight: bold;font-size: 16px;width: 46px; display: inline-block;">{{ index === 0 ? $t('Save.BranchesTabs.4265311-2') : $t('Save.BranchesTabs.4265311-3') }}</span>
          ...
          <template v-if="branch.parallel?.length">
            <div v-if="show" class="branches-tabs-title">
              {{ $t('Save.BranchesTabs.4265311-12') }}
            </div>
