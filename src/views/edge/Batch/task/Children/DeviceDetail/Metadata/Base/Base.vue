  ...
  >
    ...
    <template #id="{ record, index }">
      <EditTableFormItem :name="[index, 'id']" @change="metadataChange">
        <a-input v-model:value="record.id" placeholder="请输入标识" :disabled="true"/>
      </EditTableFormItem>
    </template>
    <template #name="{ record, index }">
      <EditTableFormItem :name="[index, 'name']" @change="metadataChange">
        <a-input v-model:value="record.name" placeholder="请输入名称" :disabled="true" />
      </EditTableFormItem>
    </template>
    <template #valueType="{ record, index }">
      <EditTableFormItem :name="[index, 'valueType']" @change="metadataChange">
        <div style="display: flex; align-items: center" v-if="['properties', 'tags'].includes(type)">
          <TypeSelect v-model:value="record.valueType" style="flex: 1 1 0;min-width: 0"  :disabled="true"/>
          <IntegerParams v-if="['int', 'long'].includes(record.valueType.type)" v-model:value="record.valueType.unit" :disabled="true"/>
          <DoubleParams v-else-if="['float', 'double'].includes(record.valueType.type)" v-model:value="record.valueType" :disabled="true"/>
          <StringParams v-else-if="record.valueType.type === 'string'" v-model:value="record.valueType" :disabled="true"/>
          <DateParams v-else-if="record.valueType.type === 'date'" v-model:value="record.valueType.format" :disabled="true"/>
          <FileParams v-else-if="record.valueType.type === 'file'" v-model:value="record.valueType.bodyType" :disabled="true"/>
          <EnumParams v-else-if="record.valueType.type === 'enum'" v-model:value="record.valueType.elements" :disabled="true"/>
          <BooleanParams
              ...
              :disabled="true"
          />
          <ObjectParams v-else-if="record.valueType.type === 'object'" v-model:value="record.valueType.properties" :disabled="true"/>
          <ArrayParams v-else-if="record.valueType.type === 'array'" v-model:value="record.valueType.elementType" :disabled="true"/>
        </div>
        <div v-else-if="type === 'events'">
          <ObjectParams v-model:value="record.valueType.properties">
            <a-button type="primary" :disabled="true" :danger="!record.valueType.properties?.length && record.id">
              <template #icon>
    ...
    <template #output="{ record, index }">
      <EditTableFormItem :name="[index, 'output']" @change="metadataChange">
        <div style="display: flex; align-items: center">
          <TypeSelect v-model:value="record.output" style="flex: 1 1 0;min-width: 0" :disabled="true"/>
          <IntegerParams v-if="['int', 'long'].includes(record.output.type)" v-model:value="record.output.unit" :disabled="true"/>
          <DoubleParams v-else-if="['float', 'double'].includes(record.output.type)" v-model:value="record.output" :disabled="true"/>
          <StringParams v-else-if="record.output.type === 'string'" v-model:value="record.output.maxLength" :disabled="true"/>
          <DateParams v-else-if="record.output.type === 'date'" v-model:value="record.output.format" :disabled="true"/>
          <FileParams v-else-if="record.output.type === 'file'" v-model:value="record.output.bodyType" :disabled="true"/>
          <EnumParams v-else-if="record.output.type === 'enum'" v-model:value="record.output.elements" :disabled="true"/>
          <BooleanParams
          ...
          <ObjectParams
              ...
              :disabled="true"
          />
          <ArrayParams v-else-if="record.output.type === 'array'" v-model:value="record.output.elementType" :disabled="true"/>
        </div>
      </EditTableFormItem>
    </template>
    <template #description="{ record }">
      <a-input v-model:value="record.description" placeholder="请输入说明" :disabled="true" @change="metadataChange"/>
    </template>
    <template #properties="{ record, index }">
      <EditTableFormItem :name="[index, 'properties']" @change="metadataChange">
        <ObjectParams v-model:value="record.valueType.properties" :disabled="true"/>
      </EditTableFormItem>
    </template>
    <template #group="{ record }">
      <GroupSelect v-model:value="record.expands.group" :disabled="true" @change="metadataChange"/>
    </template>
  </EditTable>
    <div>
