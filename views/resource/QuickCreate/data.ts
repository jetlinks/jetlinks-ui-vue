...
export const networkAndProtocol = [
  "mqtt-server-gateway",
  "websocket-server",
  "coap-server-gateway",
  "tcp-server-gateway",
...
export const queryExistProtocol = async (provider: any, data: any) => {
  const resp = await getProtocolList(ProtocolMapping.get(provider), {
...
export const queryExitPlugin = async (data: any) => {
  ...
  if (resp.success) {
    const dt = (resp.result || []).find(
      (i: any) =>
        i.configuration?.sourceId === data.id && i?.version === data?.version,
    );
    return dt;
  }
  return false;
...
// 处理插件
export const handlePluginData = async (
  resourceAccess: any,
  chooseAccess: any,
) => {
  let existPlugin;
