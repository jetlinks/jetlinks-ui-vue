...
const columns = [{
  ...
  form: {
    rules: [
      { max: 64, message: $t('Enum.ItemContent.497554-1') },
      {
        asyncValidator: (rule, value, ...setting) => {
          const option = setting[2]

          if (!value) {
            return Promise.reject($t('Enum.ItemContent.497554-2'))
          }
          ...
          if (isSome) {
            return Promise.reject($t('Enum.ItemContent.497554-3'))
          }
...
},
  {
    ...
    form: {
      rules: [
        { required: true, message: $t('Enum.ItemContent.497554-4') },
        { max: 64, message: $t('Enum.ItemContent.497554-1') },
      ]
    }
  },
  {
    title: $t('Enum.ItemContent.497554-5'),
    dataIndex: 'action',
    width: 60,
  },
]
