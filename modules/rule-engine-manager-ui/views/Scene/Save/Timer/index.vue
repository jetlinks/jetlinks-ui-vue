<template>
    <div class='timer'>
      ...
      >
        <template #label>
          <TitleComponent :data="$t('Timer.index.663670-0')" style='font-size: 14px;' />
        </template>
        <AddButton
            style='width: 100%'
            @click='visible = true'
      ...
      <div class='actions-branches-item' >
        ...
        >
          <template #label>
            <TitleComponent :data="$t('Timer.index.663670-2')" style='font-size: 14px;' />
          </template>
          <Action
...
const rules = [{
  validator(_: any, v: any) {
    if (!v) {
      return Promise.reject(new Error($t('Timer.index.663670-3')));
    }
...
const actionRules = [{
  validator(_: any, v?: BranchesThen[]) {
    if (!v || (v && !v.length) || !v.some(item => item.actions && item.actions.length)) {
      return Promise.reject($t('Timer.index.663670-4'));
    }
    return Promise.resolve();
  },
}]
