<template>
  <div class='timer-when-warp'>
    <div :class='["when-item-option", allActive ? "active" : ""]' @click='() => change(0)'>{{ $t('Timer.WhenOption.9093535-0') }}</div>
    <div
...
watch(() => props.type, () => {
  ...
  timeOptions.value = new Array(day)
    ...
    .map((_, index) => {
      ...
      return {
        label: isMonth ? $t('Timer.WhenOption.9093535-1', [_value]) : numberToString[_value],
        value: _value
      }
    })
}, { immediate: true })
