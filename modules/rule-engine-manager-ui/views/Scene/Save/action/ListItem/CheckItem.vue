...
const checkDeviceDelete = async () => {
  ...
  const proResp = await queryProductList({
    terms: [{ column: "id", termType: "eq", value: item!.productId }],
  });

  if (
    proResp.success &&
    (proResp.result as any)?.total === 0 &&
    item &&
    item.productId
  ) {
    // 产品已删除
  ...
  if (item?.selector === "fixed") {
    ...
    if (item!.selectorValues) {
      ...
      const deviceResp = await queryNoPagingPost({
        terms: [
          {
            terms: [
              { column: "id", termType: "in", value: deviceList.toString() },
            ],
          },
        ],
        paging: false
      ...
      hasDevice =
        deviceResp.success &&
        deviceResp.result.length === (item!.selectorValues?.length || 0);
      ...
      if (item!.selectorValues!.length === 1 && hasDevice) {
        ...
        metadata = JSON.parse(
          deviceDetail?.metadata || productDetail?.metadata || "{}"
        ); // 只选中一个设备，以设备物模型为准
      }
    }
    if (!hasDevice) {
      ...
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.selectorValues = undefined;
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.changeData = true;
      formTouchOff();
      return;
    }
  } else if (item!.selector === "context") {
    // 如果是按变量，校验上一个设备输出的产品id
    // if (props.name === 0) {
    ...
    const relationData = await getRelationUsers({
      paging: false,
    ...
    if (item!.selectorValues && metadata?.tags?.length) {
      const values = (item!.selectorValues?.[0]?.value as any).map(
        (item: any) => item.column
      );
      const tagKeys = new Set(values);
      hasAllTags = [...tagKeys.values()].every((_key) =>
        metadata?.tags.some((item: any) => item.id === _key)
      );
  ...
  if (item!.message!.messageType === "READ_PROPERTY") {
    ...
    if (item!.message!.properties && metadata.properties?.length) {
      ...
      hasProperties = metadata.properties?.some(
        (item: any) => item.id === propertiesKey
      );
    }
    if (!hasProperties) {
      _data.value.branches![props.branchesName].then[props.thenName].actions[
  ...
  if (item!.message!.messageType === "WRITE_PROPERTY") {
    ...
    if (!hasProperties) {
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.message!.properties = undefined;
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.changeData = true;
      formTouchOff();
    ...
    if (_value.source === "upper") {
      ...
      if (!option) {
        _data.value.branches![props.branchesName].then[props.thenName].actions[
          props.name
        ].device!.message!.properties![propertiesKey] = undefined;
        _data.value.branches![props.branchesName].then[props.thenName].actions[
          props.name
        ].device!.changeData = true;
        formTouchOff();
  ...
  if (
    item!.message!.messageType === "INVOKE_FUNCTION" &&
    item!.message!.functionId &&
    metadata.functions
  ) {
    const functionId = item!.message!.functionId;
    let hasFunctions = false;
    if (functionId && metadata.functions.length) {
      hasFunctions = metadata.functions?.some(
    ...
    if (!hasFunctions) {
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.message!.functionId = undefined;
      _data.value.branches![props.branchesName].then[props.thenName].actions[
        props.name
      ].device!.changeData = true;
      formTouchOff();
...
const checkNoticeDelete = async () => {
  ...
  const templateDetailResp = await getTemplateDetail(
    item!.templateId
  );
        ...
        if (itemData.source === "upper") {
          ...
          if (itemType === "user") {
            ...
            if (["dingTalk", "weixin"].includes(notifyType)) {
              resp =
                notifyType === "dingTalk"
                  ? await queryDingTalkUsers(item!.notifierId)
                  : await queryWechatUsers(item!.notifierId);
            } else {
              hasUser = true;
            }

            if (resp && resp.success) {
              hasUser = resp.result.some(
                (uItem: any) => uItem.id === itemData.value
              );
            }
            return hasUser;
          }
          ...
          if (itemType === "tag") {
            // 标签
            const resp = await getTags(item!.notifierId);
            if (resp && resp.success) {
              hasUser = resp.result.some(
                (tag: any) => tag.id === itemData.value
              );
            }
            return hasUser;
          }
          ...
          if (itemType === "org") {
            ...
            if (["dingTalk", "weixin"].includes(notifyType)) {
              resp =
                notifyType === "dingTalk"
                  ? await dingTalkDept(item!.notifierId)
                  : await weChatDept(item!.notifierId);
            } else {
              hasUser = true;
            }
          ...
          // 获取平台用户
          const userData = await getNotifyVariablesUser(
            _triggerType === "device"
          );
          let hasUser = false;
          if (!hasUser && userData.platform.length) {
            ...
            hasUser = userData.platform.some((uItem) => {
              return (
                uItem.id === (itemData.relation as PlatformRelation)?.objectId
              );
            });
          }
...
const check = () => {
  if (_data.value.branches![props.branchesName] && _data.value.branches![props.branchesName].then[props.thenName]) {
    ...
    if (_executor === "device" && action?.device) {
      // 设备输出，并且有值
      checkDeviceDelete();
