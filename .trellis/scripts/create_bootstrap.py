...
Arguments:
    project-type: frontend (backend/fullstack values will be treated as frontend)
...
def write_task_json(task_dir: Path, developer: str, project_type: str) -> None:
    ...
    task_data = {
        ...
        "notes": f"First-time setup task created by trellis init (frontend docs mode, requested: {project_type})",
    }
...
def main() -> int:
    ...
    # Validate project type (legacy args are accepted and mapped)
    if project_type not in ("frontend", "backend", "fullstack"):
        print(f"Unknown project type: {project_type}, defaulting to frontend")
        project_type = "frontend"
