...
def get_check_context(dev_type: str, repo_root: Path) -> list[dict]:
    ...
    if dev_type in ("frontend", "fullstack", "backend", "test"):
...
def cmd_init_context(args: argparse.Namespace) -> int:
    ...
    if not dev_type:
        ...
        print("  dev_type: frontend | fullstack | backend | test | docs")
        return 1
...
Arguments:
  dev_type: frontend | fullstack | backend | test | docs
...
Examples:
  ...
  python3 task.py init-context .trellis/tasks/01-21-add-login frontend
  python3 task.py add-context <dir> implement .trellis/spec/frontend/component-guidelines.md "Component guidelines"
...
def main() -> int:
    ...
    p_init.add_argument("type", help="Dev type: frontend|fullstack|backend|test|docs")
